...
namespace __detail
{
    ...
    void bst_impl<tkey, tvalue, compare, AVL_TAG>::post_insert(
        ...
        auto* current = static_cast<typename AVL_tree<tkey, tvalue, compare>::node*>((*node)->parent);
        ...
        while (current != nullptr)
        {
            ...
            if (abs(balance) > 1)
            {
                auto& subtree_ref = (current->parent == nullptr)
                                    ? cont._root
                ...
                if (balance > 1)
                {
                    auto* right = static_cast<decltype(current)>(current->right_subtree);

                    if (right->get_balance() < 0)
                    {
                        auto& right_ref = (right->parent->left_subtree == right)
                ...
                {
                    auto* left = static_cast<decltype(current)>(current->left_subtree);

                    if (left->get_balance() > 0)
                    {
                        auto& left_ref = (left->parent->left_subtree == left)
                ...
                current->recalculate_height();
                if (current->parent) {
                    static_cast<decltype(current)>(current->parent)->recalculate_height();
                }
            }

            current = static_cast<decltype(current)>(current->parent);
        }
    }
